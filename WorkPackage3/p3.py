...
def setup():
    ...
    GPIO.add_event_detect(btn_submit, GPIO.RISING, callback=btn_increase_pressed, bouncetime=200)
    GPIO.add_event_detect(btn_increase, GPIO.RISING, callback=btn_guess_pressed, bouncetime=200)
    pass
...
def btn_increase_pressed(channel):
    ...
    if current_guess == 7:
        ...
        current_guess -= 1
    current_guess += 1
    ##GPIO.output(11,int(bin(current_guess))) FIX
    pass
...
def btn_guess_pressed(channel):
    ...
    if button_press > 0.05
        compare_value = abs(guess-val)
        guess = guess + 1
        if compare_value == 0:
            PWM_LED.ChangeDutyCycle(0)
            PWM_buzzer.ChangeDutyCycle(0)
            print("Congratulations that was a correct guess!\nThat took you ", guess, " guess/s!")
            save_scores()
        else:
            trigger_buzzer()
            accuracy_leds()       
...
def accuracy_leds():
    ...
    bright = 0
    if guess > val:
        bright = round(((8-guess)/(8-val))*100)
    else:
        bright = round((guess/val)*100)
    PWM_LED.ChangeDutyCycle(bright)
    pass
    ...
    if compare_value == 1:
        ...
        if compare_value == 2:
            ...
            if compare_value == 3:
                ...
                frequency = 0 #not sure if this right
