...
def display_scores(count, raw_data):
    ...
    for t in range(3):
        print(t, "-", raw_data[t][0] + raw_data[t][1] + raw_data[t][2], "took", raw_data[t][3], "guesses")
    pass
...
def fetch_scores():
    ...
    score_count = None
    score_count = eeprom.read_block(0,1)[0] #not sure DOUBLE CHECK
    # Get the scores
    scores = []
    h = 1
    while eeprom.read_block(h,4) != [0,0,0,0]:
        scores.append(eeprom.read_block(h,4))
...
def btn_guess_pressed(channel):
    ...
    if 0.05 < button_press < 0.4:
        compare_value = abs(val-current_guess)
...
def accuracy_leds():
    ...
    if current_guess > val:
        bright = (8-current_guess)/(8-val)
    else:
        bright = current_guess/val
...
def trigger_buzzer():
    ...
    global PWM_buzzer
    duty_cycle = 50
    #compare_value = abs(current_guess-val)
    frequency = 1
