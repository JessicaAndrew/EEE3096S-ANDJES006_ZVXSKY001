...
void initGPIO(void){
	...
	//Set up the LED
	//Write your Logic here - set LED pin to output - done
	wiringPiSetup();
	pinMode(25,OUTPUT);
	...
	//Write your logic here - check
	pinMode(30, INPUT); 
	pullUpDnControl(30, PUD_UP);
	wiringPiISR(30,INT_EDGE_RISING,hourInc); //insert relevant callback function
...
int main(void){
	...
	for (;;){
		...
		//Write your logic here - done
			digitalWrite(25,1); 
			delay(500);
			printf("The current time is: %d:%d:%d\n", hours, mins, secs); // Print out the time we have stored on our RTC
			digitalWrite(25,0); 
			delay(500);
			//printf("The current time is: %d:%d:%d\n", hours, mins, secs);
		...
		//delay(1000); //milliseconds ///remove delay
	}
	return 0;
}
...
void hourInc(void){
	...
	if (interruptTime - lastInterruptTime>350){
		int decHours =  decCompensation(hours);
		printf("Interrupt 1 triggered, %x\n", decHours);
		//Fetch RTC Time
		//Increase hours by 1, ensuring not to overflow
		//Write hours back to the RTC - done
		hours = wiringPiI2CReadReg8(RTC, HOUR_REGISTER);
